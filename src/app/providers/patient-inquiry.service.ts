  ...
  private disableDownload: BehaviorSubject<boolean>;
    ...
    getPatientInquiry(pInquiryId): Observable<InquiryDetail> {
      this.disableDownload.next(false);
        return this.http.getHttp(`${ApiEndPoints.PATIENT_INQUIRY}/${pInquiryId}`).pipe(map(res => {
    ...
    async downloadInquiry(inquiryFileName = '', inquiryReqId, inquiryStatus) {
      ...
      if( inquiryStatus == 'pending' ) {
        this.http.putHttp(`${ApiEndPoints.REQUESTS}/${inquiryReqId}`,{'action': 'accept'}).subscribe(res=>{
          console.log('accept res>>', res);
          this.commonService.presentToast(res.message);
          this.http.getHttpVCF(`${ApiEndPoints.DOWNLOAD_INQUIRY}/${inquiryFileName}`, inquiryFileName, this.path).subscribe(res=>{
            console.log('res>>>', res)
            this.fileOpener.open(res.nativeURL, 'text/x-vcard')
    ...
    rejectRequest(inquiryReqId, confirmType) {
      this.http.putHttp(`${ApiEndPoints.REQUESTS}/${inquiryReqId}`,{'action': confirmType}).subscribe(res=>{
        console.log('accept res>>', res);
    ...
    async presentAlertConfirm(inquiryFileName = '', inquiryReqId, confirmType, inquiryStatus) {
      let confirmMesssage = ''
        ...
        buttons: [
          {
            ...
            handler: () => {
              ...
              if( confirmType == 'accept') {
                this.downloadInquiry(inquiryFileName, inquiryReqId, inquiryStatus);
              } else {

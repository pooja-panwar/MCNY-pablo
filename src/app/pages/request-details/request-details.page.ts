...
import { InquiryDetail, PatientInquiryService } from '../../providers/patient-inquiry.service';
...
export class RequestDetailsPage implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(params => {
      if (this.router.getCurrentNavigation().extras.state) {
        ...
        this.inquiryStatus = this.router.getCurrentNavigation().extras.state.inquiryStatus? this.router.getCurrentNavigation().extras.state.inquiryStatus: 'pending';
        console.log(this.inquiryId, this.inquiryStatus);
        this.pageTitle = (this.fromPage == 'notification' ||  this.fromPage == 'inquiry-list')? 'Inquiry': 'Details'
        this.rejectAction = this.inquiryStatus == 'pending'? 'reject': (this.inquiryStatus == 'active')? 'cancel': 'reject'
      }
    })
   }
  ...
  downloadInquiry() {
    this.patientInquiry.presentAlertConfirm(this.inquiryFileName, this.inquiryReqId, 'accept', this.inquiryStatus).then((res)=>{
      this.rejectAction = 'cancel';
    })
  }

  rejectInquiry() {
    this.patientInquiry.presentAlertConfirm(this.inquiryFileName, this.inquiryReqId, this.rejectAction, this.inquiryStatus).then((res)=>{
